        ...
        on_attach = function(bufnr)
            ...
            local function open_or_preview()
                ...
                if node and node.type == "file" then
                    ...
                    if name:match("%.png$") or name:match("%.jpe?g$") or name:match("%.gif$") or
                       name:match("%.webp$") or name:match("%.bmp$") or name:match("%.svg$") then
                        preview_image(node.absolute_path); return
                    end
                end
                api.node.open.edit()
            end
            vim.keymap.set("n", "<CR>", open_or_preview, { buffer = bufnr, noremap = true, silent = true, nowait = true })
